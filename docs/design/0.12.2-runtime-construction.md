...
diagnostic size failures, and multiple Wasm size tiers are separate changes.
They should be designed together because they alter diagnostic meaning and
because policy identity persistence would affect durable audit semantics. This
safety patch does not imply that the current in-memory identity is persisted.
