...
11. idempotent same-runtime bootstrap with no generation advance;
...
- Default-runtime policy is evaluated once per runtime object. Calling
  bootstrap again on an already bootstrapped object intentionally returns the
  established capability instead of applying a different policy.
